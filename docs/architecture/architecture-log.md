...
**Estado:** Resuelto. Los dos pendientes que dejó el issue #31 se cerraron en dos pasadas:

1. **`examples/` ya está cubierto por CI:** se agregó el job `examples` en `.github/workflows/ci.yml`, que compila el SDK, instala `examples/` contra ese build (dependencia `file:../sdk`, que consume el paquete por su nombre público) y corre `npm run typecheck`. Como los ejemplos importan la superficie pública, cualquier omisión futura en `sdk/src/index.ts` (u otro rompimiento de la superficie) deja el CI en rojo sin necesidad de publicar en npm. La sensibilidad quedó verificada: al quitar temporalmente el export de `CreatePaymentRequest` del `index.ts`, el typecheck falla con `TS2305 ... has no exported member`. No hace falta ejecutar los ejemplos contra el simulador en CI: para la clase de error que interesa, alcanza con que compilen.
2. **El paquete publicado ya no lleva su suite de pruebas ni archivos internos:** la compilación de publicación se separó en `sdk/tsconfig.build.json`, que extiende el `tsconfig.json` de desarrollo (que sigue incluyendo los `*.test.ts` para verificación en el editor) y excluye `src/**/*.test.ts` y `src/test-support/**` — este último no termina en `.test.ts`, así que un `exclude` solo por patrón de nombre lo habría publicado igual; `expectTransaction()` y `expectRedirect()` están deliberadamente fuera de la API pública porque desenvuelven la unión `PaymentResult` lanzando excepción, y ofrecerlo a un comercio anularía la seguridad en tiempo de compilación que la unión existe para dar. El `tsconfig.build.json` también desactiva `declarationMap`, para no publicar `.d.ts.map` que apunten a `src/`, que el tarball ya no incluye. Y `sdk/package.json` declara `files: ["dist"]`: el `tsconfig` decide qué se compila, pero `files` decide qué se publica, y sin esa lista blanca npm habría publicado `src/` completo, `scripts/ck-metrics.ts` y los archivos de configuración. `npm pack --dry-run` quedó además como paso del CI ("Verify package tarball") y hoy lista solo artefactos de distribución (`dist/**`, `package.json`, `README.md` y `LICENSE`), sin archivos de prueba ni mapas de declaración.

La mitad de empaquetado del punto 2 (el `tsconfig.build.json`, el campo `files` y el paso de verificación del tarball) se implementó dentro del alcance del issue #88, que se hizo cargo al preparar la publicación en npm; este issue cierra la parte que faltaba — el job de `examples/` en CI — para que ninguno de los dos quede sin dueño, y deja el registro del punto 21 completo.
