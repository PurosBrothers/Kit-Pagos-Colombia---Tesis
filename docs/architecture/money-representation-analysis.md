...
### Corrección de fuente: la cita de Rapyd es documentación oficial, no un hilo de foro (hallazgo de #52)

La sección "Evidencia externa de que el problema no es hipotético" atribuye la advertencia de Rapyd sobre los ceros a la derecha al foro de la comunidad ("hilo *Still having problems with fractions*"). Al implementar `RapydAdapter` (#52) se encontró que ese texto **no es de un hilo de foro: está en la página de documentación oficial de la API**, en `docs.rapyd.net/en/request-signatures.html`, dentro de la nota normativa que acompaña a la fórmula de firma de requests. La cita completa termina con un ejemplo que la versión del foro no incluía:

> "Different languages handle Base-64 encoding differently. You must adequately test your code so that all messages are encoded with all the correct Base-64 options. Python and JavaScript truncate the rightmost zeroes from decimal numbers (12.50 → 12.5), and convert decimals to integers when all digits to the right of the decimal are zero (12.00 → 12). This can sometimes cause issues with the calculation of the signature. **If you use rightmost zeroes, you can avoid these problems by sending them as numeric strings instead of JSON numbers, for example, `"12.50"` or `"12.00"`.**"
> — Rapyd, *Request Signatures* (documentación oficial de la API)

La diferencia importa para la defensa del trabajo, y conviene tenerla clara:

- Un hilo de foro es **evidencia anecdótica**: un integrador tuvo un problema. Es refutable con "a ese le pasó por implementarlo mal".
- La documentación oficial de la pasarela es **evidencia normativa**: el propio fabricante instruye a enviar los montos con ceros a la derecha como strings numéricos. Deja de ser una inferencia del equipo del proyecto y pasa a ser una exigencia documentada del proveedor.

Es decir: la decisión del issue #63 de representar `Amount` como string no fue una precaución teórica derivada de artículos genéricos sobre punto flotante, sino el cumplimiento de una recomendación explícita de una de las cuatro pasarelas que el proyecto integra, publicada por ella misma. `Amount.toFixedScale()` existe precisamente para producir el formato que esa nota pide, y `RapydAdapter` es el consumidor que lo usa.

Queda además confirmado que **son dos fórmulas de firma distintas** en Rapyd, no una: la de requests salientes incluye el método HTTP y usa el path relativo, mientras la de webhooks lo excluye y usa la URL completa configurada en el panel. La primera no estaba documentada en el proyecto; ver `architecture-log.md`, punto 27.
...
- [Rapyd — Request Signatures (documentación oficial de la API)](https://docs.rapyd.net/en/request-signatures.html): página normativa de Rapyd con la fórmula de firma de requests salientes y la nota que instruye enviar los montos con ceros a la derecha como strings numéricos (`"12.50"`, `"12.00"`) en vez de números JSON. **Es la fuente primaria que respalda la representación de `Amount` como string** (issue #63) y sustituye, con una atribución correcta, la cita que este documento atribuía a un hilo del foro. Contiene además el ejemplo de código Node.js oficial del que se confirmó que el digest del HMAC se serializa a hexadecimal antes de codificarse en Base64.
- [Rapyd — Header Parameters](https://docs.rapyd.net/en/header-parameters.html): documentación oficial de los headers obligatorios de toda petición REST a Rapyd (`access_key`, `salt`, `signature`, `timestamp`, `Content-Type`) y del `idempotency` opcional. Fuente de la fila `requestAuth` de `ubiquitous-language.md`.
- [Rapyd — Create Payment](https://docs.rapyd.net/en/create-payment.html): referencia oficial del endpoint `POST /v1/payments`. Fuente de la forma nativa de la respuesta (el sobre `{ status, data }`, el campo `currency_code` en la respuesta frente a `currency` en la petición) y del catálogo completo de estados del pago, incluido `REV` ("Reversed by Rapyd"), que resolvió un pendiente que el lenguaje ubicuo conjeturaba como `CAN`.
