...
`ubiquitous-language.md` no se corrigió como parte de este renombrado porque sus ~30 menciones de "PayU" son la columna completa de mapeo de campos contra la API clásica de PayU (solicitud de pago, webhook, consulta de estado, catálogo de errores); reemplazar solo la palabra dejaría contenido técnico falso. Ese archivo requiere el mismo nivel de investigación de campo que ya se hizo para Wompi, Mercado Pago y Kushki, sobre la API real de Rapyd Collect/Checkout, y se gestiona como un issue de GitHub nuevo, prerrequisito de la implementación del `RapydAdapter` en Iteración 2.

**Estado:** Pendiente en el SAD (todas las menciones listadas arriba) y pendiente en `ubiquitous-language.md` (requiere investigación, no solo corrección de texto). El diagrama `Component Diagram - C4.png` del SDK y el `Domain Class Diagram.png` no tienen fuente PlantUML en el repositorio, por lo que deben corregirse manualmente por sus respectivos dueños (Joshua y Henao) con la herramienta con la que se crearon originalmente.
